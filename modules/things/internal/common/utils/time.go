...
func MakeTimestamp() int {
	return time.Now().UnixNano() / int(time.Millisecond)
}

// 获取当天0时的时间戳
func Get0ClockTimeStamp(d time.Time) int {
	return time.Date(d.Year(), d.Month(), d.Day(), 0, 0, 0, 0, time.Local).UnixNano() / int(time.Millisecond)
}
